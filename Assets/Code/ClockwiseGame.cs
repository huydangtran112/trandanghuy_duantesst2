...
public class ClockwiseGame : MonoBehaviour
{
    ...
    private float rotationDirection = 0.5f;
    ...
    void Start()
    {
        // Lưu scale gốc và kích thước pixel ban đầu
    ...
    void Update()
    {
        ...
        if (!isRotating && Input.GetMouseButtonDown(0))
        {
            ...
            else
            {
                if (!dotsEmptyToggled)
                {
                    if (hit.collider != null && dotsEmpty.Contains(hit.collider.transform))
                        clickedDot = hit.collider.transform;
                    else
                    {
                        foreach (Transform dot in dots)
            ...
            Transform nearestPoint = Vector2.Distance(clickedDot.position, pointA.position) <
                                     Vector2.Distance(clickedDot.position, pointB.position)
                                     ? pointA : pointB;
            Transform oppositePoint = (nearestPoint == pointA) ? pointB : pointA;

            if (clickedIsDotEmpty && dotsEmptyToggled)
                return;
    ...
    void UpdateDotsEmptySprites()
    {
        foreach (Transform dotEmpty in dotsEmpty)
        {
            ...
            {
                sr.sprite = newSprite;
                // Đã bỏ ApplyCorrectScale để không thay đổi scale
    ...
    void UpdateDotSprites()
    {
        foreach (Transform dot in dots)
        {
            ...
            Sprite targetSprite = hasA && hasB ? bothDotSprite :
                                  (hasA || hasB ? highlightDotSprite : defaultDotSprite);
    ...
    void RotateArm()
    {
        ...
        if (reachedDot != null)
        {
            float angleToSnap = Vector2.SignedAngle(
                rotatingPoint.position - pivotPoint.position,
                reachedDot.position - pivotPoint.position);
    ...
    void CreateAfterImage()
    {
        ...
        sr.DOFade(0f, afterimageDuration).OnComplete(() =>
        {
            Destroy(afterImage);
        });
    }
